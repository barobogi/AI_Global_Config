# 📜 3AI 활성 규칙 스냅샷 (ANTI)
> 자동 생성/동기화: `43_function_dev/02_rule_governance_db/rule_governance.db` (2026-08-30 19:03)
...
| `A-04_VIDEO_TECH_QA` | **유튜브 1차 기술 스펙 및 5% Safe Zone 검증** | `anti` | `before_render` | verify_video.py 및 qa_s00_frames.py를 직접 실행하여 5/5 통과 확인할 것. |
| `A-05_NO_REDIRECT_STDOUT` | **두복이 텔레그램 리다이렉트 금지** | `anti` | `before_run` | Claude CLI 채널 모드 실행 시 표준 출력 리다이렉트를 절대 추가하지 말 것. |