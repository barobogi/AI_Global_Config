...
### 3-1. 단일 환경 멀티 에이전트 프레임워크의 근본 한계

LangGraph, AutoGen 등 기존 멀티에이전트 프레임워크는 스테이트(State)라는 공유 메모리 객체를 **단일 Python 런타임 내에서만 공유**하는 방식으로 에이전트 간 협력을 구현한다. 

**LangGraph 공식 아키텍처의 특성:**
- "스테이트라는 공유 메모리를 기반으로 단일 그래프 안에서 노드(작업자)들이 동작"
- 서브그래프(Sub-graph) 중첩 구조를 통해 복잡한 오케스트레이션 지원
- **그러나 근본적으로 동일한 프로세스 내 메모리 공유에만 국한**

이는 **이종(Cross-platform) 환경에서의 에이전트 협력을 전혀 다루지 않으며**, 상용 폐쇄형 앱(Desktop Claude 등 샌드박스 환경)과의 통합이 원천적으로 불가능하다.
...
폐쇄형 샌드박스 환경의 GUI 기반 LLM 앱과 백그라운드 자율형 CLI 에이전트들을 다음의 3계층 구조로 실시간 동기화하는 멀티 에이전트 오케스트레이션 시스템:
...
**특징**: 기존의 LangGraph 등 단일 Python 런타임 기반 멀티에이전트 프레임워크와 달리, **서로 다른 플랫폼(상용 GUI 앱 vs. 로컬 CLI)의 에이전트를 이질적 환경 그대로 유지하면서도 완벽하게 동기화** 한다.
