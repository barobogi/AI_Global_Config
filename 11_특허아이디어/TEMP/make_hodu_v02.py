...
doc.add_paragraph(
    '특히 기존 AI 시스템은 대화가 쌓여도 응답 방식이 변하지 않아 '
    '"관계가 깊어지는 느낌"을 표현하지 못한다. '
    '실제 인간 관계처럼 처음에는 무뚝뚝하다가 대화가 쌓일수록 친근해지는 '
    '친밀도 표현 강화 기능은 구현된 사례가 없다.'
)
...
goals = [
    ...
    '누적 대화량에 따라 친밀도 표현이 단계적으로 강화 (무뚝뚝 → 친근 → 애교)',
...
doc.add_paragraph()
doc.add_heading('핵심 청구항 (v0.2 업데이트)', 2)
p = doc.add_paragraph()
p.add_run(
    '"복수의 가족 구성원 음성 데이터를 장기 누적 학습하여, '
    '특정 캐릭터 페르소나로 응답을 생성하되, '
    '누적 대화량에 따라 응답의 친밀도 표현이 단계적으로 강화되는 것을 특징으로 하는 '
    'AI 인형 시스템 및 그 방법"'
).bold = True

doc.add_paragraph()
doc.add_paragraph('※ 3가지 핵심 차별점이 하나의 청구항에 통합됨:')
diff3 = [
    '가족 복수 화자 누적 학습',
    '캐릭터 페르소나 유지',
...
doc.save('D:/AI/_DEV_ITEM/호두AI_발명개념서_v0.2.docx')
print('v0.2 완료')
