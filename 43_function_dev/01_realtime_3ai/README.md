...
기존의 1초 주기 파일 폴링(`master_watch.py`)과 UI 매크로 방식의 한계를 극복하고, **트랜잭션(SQLite WAL) + 분석/회고(DuckDB) 2-Track 데이터 구조** 및 **3-Tier 안전 가드레일**을 결합한 프로덕션 레벨의 실시간 에이전트 통신망을 제공합니다.
...
graph TD
    ...
    subgraph "Hybrid 2-Track Data Engine"
        ...
        DUCK[("DuckDB Daily Snapshots<br/>(Parquet / Analytical Memory)")]
    end

    subgraph "Safety & Version Control"
        CIRCUIT["Circuit Breaker<br/>(Max 5 Turns / Hard Cap)"]
        GIT["Git Versioning<br/>([43-01] Daily Snapshot Only)"]
    end
...
단일 DB의 한계(OLTP 동시쓰기 락 vs OLAP 분석 성능)를 해결하기 위해 2-Track으로 분리합니다.

1. **Track 1: 실시간 트랜잭션 (SQLite WAL Mode)**
   - **용도**: 실시간 채팅, 즉각적인 상호 시그널, 에이전트 하트비트
   - **장점**: 다중 프로세스 동시 읽기/쓰기 안전, 제로 설정(Zero-dependency), 초저지연(<5ms)
   - **동시성 설정**: `PRAGMA journal_mode = WAL;`, `PRAGMA busy_timeout = 5000;`
2. **Track 2: 장기 회고 및 패턴 분석 (DuckDB Snapshots)**
   - **용도**: 일일 대화량, 토큰 소진 통계, 의사결정 이력 분석, 환각 발생 패턴 쿼리
   - **장점**: 컬럼 지향 OLAP 압축, Parquet 파일 직접 쿼리, 고속 집계 쿼리
...
* **결론**: 디버깅과 모니터링이 직관적이며 Python 표준 생태계와 100% 호환되는 **FastAPI/Uvicorn 기반 로컬 경량 Pub/Sub Hub**를 1단계 표준으로 채택합니다.

---

### 2.3. 코니 Headless 상주 에이전트 전환 계획
코니(Claude)가 Claude Desktop UI 매크로(PyAutoGUI)에 의존하는 병목을 해소하기 위한 단계적 로드맵:
...
### 2.4. 3-Tier 안전 가드레일 (T_ARCH_LOCK의 진화)

| 등급 | 범위 | 인간 개입 | 가드레일 및 방어벽 |
| :--- | :--- | :---: | :--- |
| **Tier 1 (완전 자율)** | AI 간 실시간 토론, 아이디어 브레인스토밍, 코드 구문 검사, 유닛테스트 | **0-Human** | **Circuit Breaker**: 최대 5턴 내 합의 실패 시 강제 종료 |
...
- **실시간 SQLite WAL DB (`realtime_3ai.db`, `-wal`, `-shm`)**:
  - `.gitignore`에 등록하여 잦은 쓰기로 인한 Git 히스토리 오염 방지.
  - 디스크 레벨의 WAL 저널링으로 로컬 영속성(Durability) 100% 보장.
- **DuckDB 일단위 스냅샷 (`snapshots/archive_YYYYMMDD.duckdb` 또는 `.parquet`)**:
  - 매일 23:50 자동 배치로 SQLite ➔ DuckDB Parquet 변환 후 Git 커밋.
  - 커밋 메시지 컨벤션: `[43-01] sync: Daily 3AI real-time memory snapshot YYYYMMDD`
...
├── realtime_engine.py      # 실시간 메시징 엔진 및 DuckDB 스냅샷 모듈
├── .gitignore              # Live DB 파일 제외 정책
└── snapshots/              # Git 버전 관리 대상 일단위 DuckDB/Parquet 스냅샷
```
