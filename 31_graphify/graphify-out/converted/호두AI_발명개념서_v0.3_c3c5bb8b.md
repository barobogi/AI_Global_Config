<!-- converted from 호두AI_발명개념서_v0.3.docx -->

호두 AI — 발명 기본 개념서
작성일: 2026-06-29  |  작성자: 이한복 (barobogi)  |  버전: v0.3

버전 이력: v0.1 초안 → v0.2 핵심 청구항 3가지 통합 → v0.3 선행기술 조사 + 청구항 구체화
...
특히 기존 AI 시스템은 대화가 쌓여도 응답 방식이 변하지 않아 관계가 깊어지는 느낌을 표현하지 못한다. 처음에는 무뚝뚝하다가 대화가 쌓일수록 친근해지는 친밀도 표현 강화 기능은 구현된 사례가 없다.
# 4. 발명의 목적
1. 가족 구성원 복수 화자의 대화 데이터를 장기 누적 학습
2. 특정 캐릭터의 페르소나를 일관되게 유지하며 응답
3. 누적 대화량에 따라 친밀도 표현이 단계적으로 강화 (L1 무뚝뚝 → L5 애착)
4. WiFi 내장 스피커/마이크 모듈을 인형 외형에 통합한 독립 기기 구현
5. 모바일 앱과 연동하여 기기 무관 동일 AI 페르소나 연속 유지 (v0.3 추가)
# 5. 핵심 구성요소
[캐릭터 인형 외형]
       ↕
[WiFi 내장 스피커 + 마이크 모듈]
...
## 6-1. 친밀도 레벨 산출 함수
I(u,t) = 0.4 × C_u + 0.3 × S_u + 0.2 × T_u + 0.1 × R_u
C: 누적 대화 횟수  |  S: 평균 감정 점수  |  T: 주제 다양성  |  R: 최근성 가중치

## 6-2. 선행기술 조사 결과 (v0.3 신규)
선행기술 공백 — 아래 조합은 미발견:
